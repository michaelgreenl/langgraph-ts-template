...
function checkLanggraphPaths() {
    ...
    const langgraphPath = path.join(root, 'langgraph.json');
    ...
    try {
        ...
        for (const [key, value] of Object.entries(graphs)) {
            ...
            if (!isObjectExported(fullPath, objectName)) {
                console.error(`Object "${objectName}" is not exported from ${fullPath}`);
                hasError = true;
            }
        }
        ...
        if (hasError) {
            ...
            console.log('All paths in langgraph.json are valid and objects are exported correctly.');
        }
    } catch (error) {
